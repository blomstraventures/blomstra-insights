/**
 * Critical Infrastructure Vulnerability Index – Global Edition v3.1
 *
 * v3.1 fixes a real bug in v3.0's daily cron path: cii_build_from_central_cache()
 * was a second, independent composite implementation that diverged from
 * cii_build_composite() in ways that produced silently wrong published data
 * every night:
 *   - Energy was a naive average of raw consumption quantities across fuels,
 *     not the consumption-weighted dependency formula CII actually uses
 *     (cii_eia_aggregate_energy_dependency) — a different number, not just a
 *     shortcut.
 *   - Maritime read blomstra_get_maritime_raw() directly, which has no entries
 *     for landlocked countries, so every landlocked country was silently
 *     dropped from the cron-built composite (they should score a structural 0,
 *     and be counted, not omitted).
 *   - Partial-coverage rank_display used a fixed 25th/75th-percentile-of-count
 *     approximation instead of the documented OECD/JRC injection method
 *     (best-estimate/80%-range/theoretical-bound) cii_build_composite() uses.
 *   - Its output dropped methodology_url/methodology_summary/footnote/
 *     global_averages_informational_only/weights/excluded_detail, so the
 *     public REST payload's shape silently changed every night, then silently
 *     changed back on the next manual "Refresh All & Build".
 *
 * Fix: cii_build_from_central_cache() is removed. The daily cron now calls the
 * SAME cii_build_composite() the manual path uses — one methodology
 * implementation, not two — after refreshing each pillar from whatever the
 * Reference Data layer already has cached (new 'central_cached' source: reads
 * blomstra_get_eia_raw_data()/blomstra_get_comtrade_hhi_data()/
 * blomstra_get_maritime_raw(), triggers no live collection itself, per the
 * "no fallback/no API calls in cron" intent v3.0 already stated but didn't
 * actually implement this way).
 *
 * v3.1.1 fixes a second real bug found right after v3.1 went live: the
 * freshness gate (cii_check_dual_freshness) and the "Pipeline Health"
 * dashboard card both read blomstra_cron_status — a log written ONLY by
 * Reference Data's own weekly wp-cron events (Maritime=Mon, EIA=Tue,
 * HHI=Wed). A pillar refreshed successfully through CII's OWN "Central
 * Data"/"Direct API" buttons — fully scored, real data sitting in
 * cii_maritime_pillar — still showed "never refreshed" and permanently
 * blocked the cron build, because that manual refresh never touches
 * Reference Data's cron log. Fixed: both now read cii_pillar_last_refreshed(),
 * which checks the 'last_updated' timestamp CII's own pillar options already
 * carry on every successful refresh (central, central_cached, or api) — no
 * dependency on Reference Data's unrelated cron schedule.
 *
 * Also added: a "Force Run Daily Cron Now (Test)" admin button that runs the
 * exact same function the real wp-cron hook calls (cii_run_daily_build_logic,
 * freshness gate included) — no more waiting for wp-cron.php or WP-CLI to
 * verify the cron path actually works.
 *
 * Everything else from v3.0/v3.1 is preserved unchanged: self-healing build
 * lock (5 min TTL), first-run grace, all three fallback engines
 * (Energy/Maritime/HHI) for manual admin use.
 *
 * v3.1.2 adds a simpler alternative to digging through wp-config.php or
 * hosting cron settings: the daily cron now stamps cii_last_wpcron_fired the
 * moment the real wp-cron event fires — a signal separate from
 * cii_cron_status, which the "Force Run Now" test button also writes to (so
 * clicking that button can't hide a genuinely broken schedule anymore). If
 * that timestamp goes stale (>30h) or has never been set, an admin-wide
 * notice fires telling you to refresh manually via Central Data/Direct API,
 * plus a "Last Real wp-cron Fire" card on the CII page itself. If
 * wp_get_environment_type() isn't "production" (the v3.1.1 gap), that's
 * called out explicitly instead of just going quiet.
 *
 * Version: 3.1.2
 */
...
add_action( 'admin_notices', function () {
    if ( ! function_exists( 'blomstra_get_global_country_list' ) ) {
        echo '<div class="notice notice-warning"><p><strong>CII Index:</strong> the "Blomstra Reference Data" utility snippet isn\'t active. CII will still work using its own fallback API calls, but activating that snippet lets CII share its collected data (and any future fixes to it) with other index tools instead of duplicating the work.</p></div>';
    }
} );
...
if ( ! function_exists( 'cii_hhi_reporter_map_fallback' ) ) {
    function cii_hhi_reporter_map_fallback() {
        ...
        if ( is_wp_error( $response ) ) {
            error_log( 'CII reporter-map fallback fetch failed: ' . $response->get_error_message() );
            return array();
        }
...
// ── Energy fallback functions (identical to v2.9.23) ──
...
if ( ! function_exists( 'cii_eia_fetch_activity_batch_fallback' ) ) {
    function cii_eia_fetch_activity_batch_fallback( $country_codes, $activity_id, $product_id, $attempt = 1 ) {
        ...
        if ( $fail_reason !== '' ) {
            error_log( 'CII EIA batch fetch FAILED (' . $chunk_label . ', attempt ' . $attempt . '): ' . $fail_reason );
...
// ─── ENERGY: PRIMARY (CENTRALIZED RAW DATA) + FALLBACK DISPATCHER ──
...
if ( ! function_exists( 'cii_refresh_energy_pillar' ) ) {
    function cii_refresh_energy_pillar( $countries = null, $source = 'auto' ) {
        ...
        if ( $source === 'central_cached' ) {
            // Reads whatever Reference Data already has — triggers no live
            // collection itself. Used by the daily cron so it never makes its
            // own API calls; Reference Data's own weekly cron is what keeps
            // this cache warm.
...
if ( ! function_exists( 'cii_fetch_maritime_raw_fallback' ) ) {
    function cii_fetch_maritime_raw_fallback( $attempt = 1 ) {
        ...
        if ( is_wp_error( $response ) ) {
            error_log( 'CII Maritime fallback fetch WP_Error: ' . $response->get_error_message() );
            return array();
        }
        ...
        if ( $http_code !== 200 ) {
            error_log( 'CII Maritime fallback fetch HTTP ' . $http_code );
            return array();
        }

        $body = json_decode( $body_raw, true );
        if ( ! isset( $body[1] ) || ! is_array( $body[1] ) ) {
            error_log( 'CII Maritime fallback fetch: unexpected response shape' );
            return array();
        }
...
if ( ! function_exists( 'cii_refresh_maritime_pillar' ) ) {
    function cii_refresh_maritime_pillar( $source = 'auto' ) {
        ...
        if ( $source === 'central' ) {
            ...
            // Reads whatever Reference Data already has cached — does not
            // force a live fetch. Used by the daily cron.
...
if ( ! function_exists( 'cii_comtrade_fetch_partner_imports_batch_fallback' ) ) {
    function cii_comtrade_fetch_partner_imports_batch_fallback( $reporter_codes, $year, $attempt = 1 ) {
        ...
        do {
            ...
            if ( is_wp_error( $response ) ) {
                $fail_reason = 'network error: ' . $response->get_error_message();
                error_log( 'CII Comtrade batch fetch FAILED (' . $chunk_label . ', year ' . $year . ', page ' . $page . ', attempt ' . $attempt . '): ' . $fail_reason );
            ...
            if ( $code === 429 || $code === 403 ) {
                ...
                if ( preg_match( '/[Tt]ry again in\s+(\d+)\s+seconds?/', $body_snip, $m ) && (int) $m[1] <= 90 && $attempt <= 2 ) {
                    $wait = (int) $m[1] + 2;
                    error_log( 'CII Comtrade batch short-term rate limit (' . $chunk_label . ', year ' . $year . '): waiting ' . $wait . 's then retrying (attempt ' . $attempt . ')' );
                ...
                error_log( 'CII Comtrade batch fetch FAILED (' . $chunk_label . ', year ' . $year . ', attempt ' . $attempt . '): ' . $fail_reason );
            ...
            if ( strlen( $raw_body ) > 15 * 1024 * 1024 ) {
                ...
                error_log( 'CII Comtrade batch fetch SKIPPED (' . $chunk_label . ', year ' . $year . ', page ' . $page . '): ' . $fail_reason );
            ...
            if ( isset( $body['error'] ) && $body['error'] !== '' ) {
                error_log( 'CII Comtrade batch API error field (' . $chunk_label . ', year ' . $year . '): ' . $body['error'] );
                cii_log_comtrade_call( $chunk_label, $year, 'api_error', (string) $body['error'] );
                return null;
            }
            if ( ! isset( $body['data'] ) || ! is_array( $body['data'] ) ) {
                error_log( 'CII Comtrade batch unexpected response shape (' . $chunk_label . ', year ' . $year . '): missing data array' );
...
if ( ! function_exists( 'cii_refresh_hhi_pillar' ) ) {
    function cii_refresh_hhi_pillar( $year = null, $iso3_list = null, $source = 'auto' ) {
        ...
        if ( $source === 'central_cached' ) {
            // Pure option read — zero API calls. Used by the daily cron.
...
// ─── COMPOSITE BUILDER (modified to inject _meta) ─────────────────
...
if ( ! function_exists( 'cii_acquire_build_lock' ) ) {
    function cii_acquire_build_lock() {
        ...
        if ( false !== $lock_data && is_numeric( $lock_data ) && ( $now - $lock_data ) < CII_LOCK_TTL ) {
            error_log( 'CII: Build lock already held (age ' . ( $now - $lock_data ) . 's), aborting.' );
            return false;
        }

        set_transient( $lock_key, $now, CII_LOCK_TTL );
        if ( false !== $lock_data ) {
            error_log( 'CII: Force‑cleared stale lock (age ' . ( $now - $lock_data ) . 's).' );
        }
        return true;
    }
}
...
// 2. Freshness check
//
// v3.1 fix: this used to read blomstra_cron_status, which is ONLY written by
// Reference Data's own weekly wp-cron events (Mon/Tue/Wed). That meant a
// pillar refreshed successfully through CII's own "Central Data"/"Direct API"
// buttons — fully scored, real data sitting in cii_maritime_pillar etc. —
// still reported "never refreshed" until Reference Data's own cron happened
// to fire on its fixed weekly day. The freshness gate now checks CII's own
// pillar data directly (every successful refresh path — central,
// central_cached, api — stamps 'last_updated' on every row), so freshness
// reflects what's actually sitting in CII's pillars, not an unrelated
// external schedule.
...
// 3. Cron-safe build: refresh each pillar from whatever Reference Data
// already has cached (no live API calls), then run the ONE real composite
// builder — cii_build_composite() — so cron and manual "Refresh All & Build"
// always compute the index the same way and produce the same output shape.
...
// Actual cron logic, pulled into its own function so both the real wp-cron
// hook and an admin "Force Run Now" test button run identically — no
// separate/divergent test path.
if ( ! function_exists( 'cii_run_daily_build_logic' ) ) {
    function cii_run_daily_build_logic() {
        ...
        // cii_build_composite() already persisted cii_composite_index with
        // the full standard shape (methodology fields, weights, excluded
        // detail, etc.) — nothing more to save here.
        $status = array(
            'time'    => current_time( 'mysql' ),
            'status'  => 'success',
...
if ( ! function_exists( 'cii_daily_cron_callback' ) ) {
    function cii_daily_cron_callback() {
        // Stamped unconditionally the moment the real wp-cron event actually
        // fires — separate from cii_cron_status, which the "Force Run Now"
        // admin button also writes to. This is the one signal that can't be
        // faked by clicking the test button, so it's what the staleness
        // alert below checks.
...
if ( ! defined( 'CII_WPCRON_ALERT_THRESHOLD' ) ) {
    // Daily schedule + a buffer for wp-cron's page-load-triggered timing.
...
if ( ! function_exists( 'cii_wpcron_health_notice' ) ) {
    add_action( 'admin_notices', function() {
        ...
        if ( $env_skip !== null ) {
            echo '<div class="notice notice-error"><p><strong>CII Index:</strong> the daily automatic build IS firing, but is being skipped every time because wp_get_environment_type() returns "' . esc_html( $env_skip ) . '", not "production". Fix WP_ENVIRONMENT_TYPE in wp-config.php, or use the 🔄 Central Data / 🔄 Direct API buttons on the CII Index page to refresh manually until then.</p></div>';
            return;
        }
        ...
        if ( $age === null || $age > CII_WPCRON_ALERT_THRESHOLD ) {
            ...
            echo '<div class="notice notice-error"><p><strong>CII Index:</strong> the daily automatic build ' . esc_html( $detail ) . ' wp-cron may not be triggering on this site (common on low-traffic sites, or if a host runs a real system cron against wp-cron.php and it isn\'t configured). Until that\'s fixed, go to the CII Index page and use 🔄 Central Data / 🔄 Direct API on each pillar, or the "Force Run Daily Cron Now" button, to refresh manually.</p></div>';
        }
    } );
}
...
if ( ! function_exists( 'cii_health_dashboard_notice' ) ) {
    add_action( 'admin_notices', function() {
        ...
        echo '<div class="notice ' . $class . ' is-dismissible"><p><strong>CII Index:</strong> ' . esc_html( $status['details'] ) . ' (Last run: ' . esc_html( $status['time'] ) . ')</p></div>';
    } );
}
// ─── ADMIN PAGE (EXTENDED WITH HEALTH DASHBOARD) ──────────────────
...
if ( ! function_exists( 'cii_handle_early_actions' ) ) {
    function cii_handle_early_actions() {
        ...
        if ( isset( $_POST['cii_flush_energy'] ) && check_admin_referer( 'cii_flush_energy_action', 'cii_flush_energy_nonce' ) ) {
            ...
            wp_safe_redirect( add_query_arg( array( 'page' => 'cii-index', 'flushed' => 'energy' ), admin_url( 'admin.php' ) ) );
            exit;
        }
        ...
        if ( isset( $_POST['cii_flush_maritime'] ) && check_admin_referer( 'cii_flush_maritime_action', 'cii_flush_maritime_nonce' ) ) {
            ...
            wp_safe_redirect( add_query_arg( array( 'page' => 'cii-index', 'flushed' => 'maritime' ), admin_url( 'admin.php' ) ) );
            exit;
        }
        ...
        if ( isset( $_POST['cii_flush_hhi'] ) && check_admin_referer( 'cii_flush_hhi_action', 'cii_flush_hhi_nonce' ) ) {
            ...
            wp_safe_redirect( add_query_arg( array( 'page' => 'cii-index', 'flushed' => 'hhi' ), admin_url( 'admin.php' ) ) );
            exit;
        }
        ...
        if ( isset( $_POST['cii_flush_all'] ) && check_admin_referer( 'cii_flush_all_action', 'cii_flush_all_nonce' ) ) {
            ...
            wp_cache_flush();
            wp_safe_redirect( add_query_arg( array( 'page' => 'cii-index', 'flushed' => 'all' ), admin_url( 'admin.php' ) ) );
            exit;
        }
        ...
        if ( isset( $_POST['cii_do_refresh_all'] ) && check_admin_referer( 'cii_refresh_all_action', 'cii_refresh_all_nonce' ) ) {
            ...
            wp_cache_flush();
            wp_safe_redirect( add_query_arg( array( 'page' => 'cii-index', 'all_done' => 1 ), admin_url( 'admin.php' ) ) );
            exit;
        }

        if ( isset( $_POST['cii_force_daily_cron'] ) && check_admin_referer( 'cii_force_daily_cron_action', 'cii_force_daily_cron_nonce' ) ) {
            // Runs the exact same logic as the real wp-cron event (freshness
            // gate included) — just without waiting for wp-cron to fire, and
            // without the production-environment guard, so it works for
            // testing on staging too. Result lands in cii_cron_status, same
            // place the real cron writes to.
            ignore_user_abort( true );
            set_time_limit( 300 );
            cii_run_daily_build_logic();
            wp_cache_flush();
            wp_safe_redirect( add_query_arg( array( 'page' => 'cii-index', 'cron_forced' => 1 ), admin_url( 'admin.php' ) ) );
            exit;
        }
    }
...
// ─── RENDER PAGE (with Health Dashboard card) ──────────────────────
...
if ( ! function_exists( 'cii_render_test_tools_page' ) ) {
    function cii_render_test_tools_page() {
        ...
        echo '<div class="wrap">';
        echo '<h1>' . esc_html__( 'Critical Infrastructure Index – Global', 'cii' ) . '</h1>';
        echo '<p style="color:#666;">Manage the CII data pipeline — refresh pillars, build the composite, and preview results.</p>';
        ...
        // Pillar ages — v3.1: reads CII's own pillar data (cii_pillar_last_refreshed),
        // not blomstra_cron_status, so this matches cii_check_dual_freshness()
        // exactly and won't show "Never refreshed" for a pillar CII refreshed
        // itself via its own Central Data / Direct API buttons.
        $pillar_options = array(
        ...
        // Real wp-cron firing status — distinct from the Force button, which
        // can make the composite look fresh even if the schedule itself is
        // broken. This card only moves when the actual cii_daily_cron event
        // fires.
        ...
        echo '<p style="color:#666;">Raw per-fuel data is centralized; weighting formula stays here (CII-specific methodology).</p>';
        ...
        echo '<form method="post" onsubmit="return confirm(\'This deletes all stored CII data. Are you sure?\');">';
        ...
        echo '<div class="inside">';
        echo '<p style="color:#666;">These are specific to CII\'s Direct API fallback path. Full diagnostics for the central model are on the <a href="' . esc_url( admin_url( 'admin.php?page=blomstra-insights-tools' ) ) . '">Blomstra Reference Data</a> page.</p>';
