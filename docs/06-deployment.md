...
Two independent signals track cron health:
...
Created automatically on first admin page load via `dbDelta()`.
...
**Upsert semantics:** `INSERT ... ON DUPLICATE KEY UPDATE`. Rebuilding the same index twice in one month updates the existing row.
...
| Rank changes when filtering | Frontend bug (old versions) | Ensure rank comes from API, not array index |
| Admin page 500 error | PHP memory limit | Increase `memory_limit` in php.ini |
| Build lock stuck | Previous build crashed | Wait 5 minutes (TTL) or manually clear transient |
...
**Note:** All raw data can be re-fetched from external APIs. The only irreplaceable data is `wp_blomstra_index_history` (historical snapshots).
