...
1. **Every indicator MUST carry provenance metadata.** The minimum required fields are:
   ...
   - `scope` — fiscal/administrative scope if applicable (e.g., `central_gov`, `general_gov`)
...
5. **Per-indicator source tracking is mandatory.** Use `blomstra_track_source()` from `index-utilities.php` to record provenance at the indicator level, not the pillar level.

### Reference Implementation
GERI's `data_freshness` object in `geri_build_composite()`, enhanced with `blomstra_track_source()` for per-indicator provenance.
...
**Critical rule:** Use `blomstra_safe_numeric()` for all numeric extraction. `empty(0.0)` returns `true`, which would misclassify structural zeros as missing.
...
5. **Outlier handling:** For indicators with heavy-tailed distributions (inflation, debt, commodity prices), winsorization at the 1st/99th percentile SHOULD be applied using `blomstra_compute_percentile_ranks_safe()`.

6. **Alternative methods (z-score, min-max) MAY be used with documented justification.** Any deviation from percentile ranking MUST be recorded in `deviations.md`.

### Reference Implementation
`blomstra_compute_percentile_ranks()` in Reference Data (backward compatibility).
`blomstra_compute_percentile_ranks_safe()` in `index-utilities.php` (recommended for new indices).
...
6. **Definitions and engine thresholds MUST stay synchronized.** Use `blomstra_validate_pillar_thresholds()` at initialization to detect drift between `index_get_pillar_defs()` and `index_get_pillar_weights()`.
...
4. **Historical windows:**
   ...
   - Minimum observations: 4 for volatility, 4 for trajectory (CAGR computation)

5. **Trajectory computation:** Use compound annual growth rate (CAGR), not absolute point difference. Use `blomstra_compute_cagr()` from `index-utilities.php`.
   ```
   CAGR = ((newest / oldest) ^ (1 / span)) - 1
   ```
   This normalizes trajectory by initial debt level and prevents small-base countries from being treated identically to large-base countries.

6. **Vintage:** Datasets with explicit versions (e.g., IMF WEO April 2026) MUST record the vintage. Datasets revised in place (e.g., World Bank WDI) record `null` or `"live"`.

7. **The `reference_vintage` field in API output MUST reflect the actual data vintage**, not a hardcoded year.

8. **Timeseries MUST be sanitized before use.** Use `blomstra_sanitize_timeseries()` to ensure chronological order and enforce minimum spans.
...
## BMS-008: Fallback & Scope Standard

### Purpose
Ensure fallback data and mixed-scope data are handled transparently.

### Rules

1. **Fallback MUST be tracked per-indicator.** Use `blomstra_merge_with_fallback()` from `index-utilities.php`. Never merge fallback data with manual `if (!isset)` logic.

2. **Scope MUST be recorded.** When fiscal data comes from different scopes (central government vs. general government), the scope MUST be stored alongside the value.

3. **Mixed scope within a pillar MUST be flagged.** If a country's fiscal pillar combines central gov debt (WB) and general gov balance (IMF), `scope_mixed` MUST be `true` in the output.

4. **Mixed scope SHOULD trigger a quality warning.** Consumers should know when a pillar combines incomparable definitions.

5. **Primary source priority SHOULD be documented.** For fiscal data, general government (IMF) is preferred over central government (WB) for cross-country comparability. Document the hierarchy in the index methodology.

### Reference Implementation
`blomstra_merge_with_fallback()` and `blomstra_pillar_source_summary()` in `index-utilities.php`.
...
- The CII case study → [`10-methodology-deepdive.md`](10-methodology-deepdive.md)
